...
// ============================================================
// window.storage SHIM — REPLACE BEFORE SHIPPING
// ============================================================
// The original artifact ran inside Claude.ai, which provides a real
// cross-device window.storage API for free (that's how the global wire,
// trade board, and leaderboard worked). That API does not exist outside
// Claude.ai. This shim below keeps the app from crashing by falling back
// to per-device localStorage — meaning global chat/trade/leaderboard will
// ONLY be visible on the same phone, not actually shared between players.
//
// To restore real multiplayer, swap this shim for calls to a real backend
// — Firebase Realtime Database or Supabase are the easiest free options —
// and point get/set/delete/list at that instead. Search this file for
// every `window.storage.` call (global_chat, trade_board, leaderboard) and
// redirect them the same way.
...
const ASSET_CATEGORIES = [
  ...
  {
    id: "office", label: "Crew Headquarters", color: "#9c8b6f", level: 4,
    items: [
  ...
  {
    id: "luxury_biz", label: "Luxury Trade Business", color: "#e8dcc8", level: 9,
    items: [
...
// ---------- factions ----------
const FACTIONS = [
  { id: "commission", name: "The Commission", icon: "♠️", color: "#c81e3a", desc: "Old-guard families that run this city's rules." },
...
function AssetTile({ icon, name, sub, price, owned, accent, onBuy, disabled, locked }) {
  return (
    <div style={{
      display: "flex", flexDirection: "column", borderRadius: 12, overflow: "hidden",
      border: `1px solid ${accent}55`, position: "relative", opacity: locked ? 0.4 : 1,
      background: `linear-gradient(150deg, ${accent}33 0%, #150f0a 55%, #0a0705 100%)`,
    }}>
      {owned > 0 && <div style={{ position: "absolute", top: 8, right: 8, background: accent, color: "#0e0a07", fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, fontSize: 10, borderRadius: 10, padding: "1px 7px", zIndex: 2 }}>×{owned}</div>}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 74, position: "relative" }}>
        <div style={{ position: "absolute", width: 60, height: 60, borderRadius: "50%", background: `radial-gradient(circle, ${accent}55, transparent 70%)`, filter: "blur(2px)" }} />
        <div style={{ fontSize: 34, filter: "drop-shadow(0 2px 6px rgba(0,0,0,0.6))" }}>{icon}</div>
      </div>
      <div style={{ padding: "8px 10px 10px", background: "rgba(10,7,5,0.55)", borderTop: `1px solid ${accent}33` }}>
        <div style={{ fontFamily: "'Cinzel', serif", fontSize: 12, color: "#e8dcc8", lineHeight: 1.25 }}>{name}</div>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: "#8a8078", marginTop: 3, marginBottom: 8 }}>{sub}</div>
        <button onClick={onBuy} disabled={disabled || locked} style={{
          width: "100%", padding: "6px 0", background: disabled || locked ? "#241a13" : accent,
...
function Modal({ children, width = 340 }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(5,3,2,0.82)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: width, background: "#140e0a", border: "1px solid #d4af3766", borderRadius: 10, padding: 22, boxShadow: "0 0 40px rgba(0,0,0,0.6)", maxHeight: "85vh", overflowY: "auto" }}>{children}</div>
    </div>
  );
}
...
function TradingChart({ data, color, width = 320, height = 140 }) {
  ...
  return (
    <svg width={width} height={height} style={{ display: "block" }}>
      {[0.25, 0.5, 0.75].map((f) => <line key={f} x1={pad} x2={width - pad} y1={pad + f * (height - pad * 2)} y2={pad + f * (height - pad * 2)} stroke="#2a2018" strokeWidth="1" />)}
      <polygon points={area} fill={`${color}22`} />
      <polyline points={line} fill="none" stroke={color} strokeWidth="2" />
      <text x={pad} y={pad + 10} fill="#8a8078" fontSize="9" fontFamily="'JetBrains Mono', monospace">{max.toFixed(2)}</text>
      <text x={pad} y={height - pad - 2} fill="#8a8078" fontSize="9" fontFamily="'JetBrains Mono', monospace">{min.toFixed(2)}</text>
    </svg>
  );
}
...
export default function Omerta() {
  ...
  async function postTradeOffer() {
    ...
    const offer = { id: Date.now(), from: PLAYER_NAME, item: tradeDraft.item.trim(), price: Number(tradeDraft.price) };
    try {
  ...
  lbTickRef.current = { cash, level, rank };
  useEffect(() => {
    const t = setInterval(async () => {
      try {
        ...
        const { cash: c, level: lv, rank: rk } = lbTickRef.current;
        filtered.push({ id: PLAYER_ID, name: PLAYER_NAME, cash: c, level: lv, title: rk.title });
  ...
  function doCrime(crime) {
    ...
    setTimeout(() => {
      ...
      if (caught) {
        ...
        const arrestChance = Math.max(0.02, baseArrest - guardArrestCut);
        if (Math.random() < arrestChance) {
          const seized = Math.round(cash * 0.4);
          setCash((c) => c - seized);
          const term = Math.round((6 + crime.level * 2.5) * (1 + priorArrests * 0.35));
          setJailTicks(term);
          ...
          const dmg = Math.round((8 + Math.random() * 15) * (1 - guardDmgCut));
          setHealth((h) => clamp(h - dmg, 0, 100));
          setHeat((h) => clamp(h + crime.heat * 1.6 * (1 - fixerCut), 0, 100));
          pushLog(`"${crime.name}" went sideways — you took a beating (-${dmg} health).`, "bad");
        }
      } else {
  ...
  function scrollTabs(dir) { if (tabBarRef.current) tabBarRef.current.scrollBy({ left: dir * 160, behavior: "smooth" }); }
  ...
  function openMeeting() {
    ...
    setActiveMeeting(MEETING_TEMPLATES[Math.floor(Math.random() * MEETING_TEMPLATES.length)]);
  }
  ...
  const toneColor = { good: "#7fae6b", bad: "#c14b3f", warn: "#d4af37", neutral: "#8a8078" };

  return (
    <div style={{ minHeight: "100vh", background: "#0e0a07", backgroundImage: "radial-gradient(ellipse at top, #1a120c 0%, #0e0a07 60%)", color: "#e8dcc8", fontFamily: "'Cormorant Garamond', serif", paddingBottom: 84 }}>
      ...
      {activeStory && (
        <Modal width={380}>
          ...
          <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 30, color: "#c81e3a", margin: "4px 0 12px" }}>{activeStory.title}</div>
          <div style={{ fontSize: 16, lineHeight: 1.65, color: "#d8c9ac", fontStyle: "italic" }}>{activeStory.text}</div>
          <button onClick={dismissStory} style={{ marginTop: 18, width: "100%", padding: "11px 0", background: "#c81e3a", color: "#e8dcc8", border: "none", borderRadius: 6, fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>CONTINUE</button>
        </Modal>
      )}
      ...
      {swatWarning && (
        <Modal width={320}>
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontSize: 14, color: "#d8c9ac", marginBottom: 16 }}>Heat's too high — a tactical unit is watching your territory. Cool off or grease someone, fast.</div>
            <button onClick={() => setSwatWarning(false)} style={{ width: "100%", padding: "11px 0", background: "#c81e3a", color: "#e8dcc8", border: "none", borderRadius: 6, fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>UNDERSTOOD</button>
          </div>
        </Modal>
      )}
          ...
          <Modal width={360}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 20, color: "#e8dcc8" }}>{activeMeeting.title}</div>
              </div>
            </div>
            <div style={{ fontSize: 15, lineHeight: 1.6, color: "#d8c9ac", marginBottom: 16 }}>{activeMeeting.text}</div>
            {meetingResult ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {activeMeeting.choices.map((c, i) => (
                  <button key={i} onClick={() => resolveMeeting(c)} style={{ padding: "10px 12px", background: "rgba(212,175,55,0.08)", border: "1px solid #d4af3755", borderRadius: 6, color: "#e8dcc8", textAlign: "left", cursor: "pointer", fontFamily: "'Cinzel', serif", fontSize: 12.5 }}>{c.label}</button>
                ))}
      ...
      {hackTarget && (
        <Modal width={360}>
          <div style={{ borderRadius: 8, overflow: "hidden", border: "1px solid #2a2018" }}>
            <div style={{ background: "#1a1a1a", padding: "7px 10px", display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <div style={{ marginLeft: 8, fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#8a8078" }}>wire@omerta — {hackTarget.name}</div>
            </div>
          ...
          <div style={{ fontSize: 11.5, color: "#6b6152", margin: "10px 0", fontStyle: "italic", textAlign: "center" }}>Fictional terminal mini-game — flavor output only, no real exploit techniques.</div>
          <button onClick={hackTap} style={{ width: "100%", padding: "13px 0", background: "#4fb8c9", color: "#0e0a07", border: "none", borderRadius: 6, fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>▶ RUN NEXT STAGE ({hackPulse}/{hackTarget.diff + 2})</button>
          <button onClick={() => { setHackTarget(null); setHackPulse(0); setHackLines([]); }} style={{ width: "100%", marginTop: 8, padding: "9px 0", background: "transparent", color: "#6b6152", border: "1px solid #2a2018", borderRadius: 6, fontSize: 12, cursor: "pointer" }}>Disconnect</button>
        </Modal>
      )}
      ...
      {jailTicks > 0 && (
        <Modal width={320}>
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, color: "#9c8b6f", marginBottom: 6 }}>{jailTicks * 2}s remaining</div>
            ...
            <button onClick={payBail} style={{ width: "100%", padding: "11px 0", background: "#c81e3a", color: "#e8dcc8", border: "none", borderRadius: 6, fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>POST BAIL — ${fmt(jailTicks * 260)}</button>
            <div style={{ fontSize: 12, color: "#6b6152", marginTop: 10, fontStyle: "italic" }}>Or wait it out. Your fronts keep running without you.</div>
          </div>
        </Modal>
      )}

      {/* compact top bar — small, always visible, doesn't push tab content down */}
      <div style={{ padding: "12px 14px", borderBottom: "1px solid #2a2018", display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 34, height: 34, borderRadius: 8, background: "#1a120c", border: "1px solid #d4af3755", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, flexShrink: 0 }}>{avatar}</div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: "'Cinzel', serif", fontWeight: 900, fontSize: 15, color: "#c81e3a", letterSpacing: "0.04em" }}>OMERTÀ</div>
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: "#6b6152", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>LVL {level} · {rank.title} · {CITIES.find((c) => c.id === currentCity)?.name}</div>
        </div>
        <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 20, color: "#7fae6b" }}>${fmt(cash)}</div>
        <button onClick={() => setLedgerOpen((o) => !o)} style={{ background: "none", border: "1px solid #3a2c20", borderRadius: 6, color: "#9c8b6f", width: 30, height: 30, cursor: "pointer", fontSize: 13 }}>{ledgerOpen ? "▲" : "▾"}</button>
      </div>

      {ledgerOpen && (
        <div style={{ padding: "0 18px 14px", borderBottom: "1px solid #2a2018" }}>
          <StampBar pct={nextRank ? ((xp - rank.xp) / (nextRank.xp - rank.xp)) * 100 : 100} color="#d4af37" />
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#5c5142", margin: "6px 0 10px" }}>{fmt(xp - rank.xp)} / {nextRank ? fmt(nextRank.xp - rank.xp) : "MAX"} XP to {nextRank?.title || "the top"} · Playing as {PLAYER_NAME}</div>
          <div style={{ background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 4, padding: "4px 12px" }}>
            ...
            <Ledger label="Health" value={`${Math.round(health)}/100`} accent={health < 30 ? "#c14b3f" : "#e8dcc8"} />
            <Ledger label="Energy" value={`${Math.round(energy)}/100`} accent="#7fa8c9" />
            <Ledger label="Heat" value={`${Math.round(heat)}%`} sub={heat > 60 ? "getting dangerous" : "manageable"} accent={heat > 60 ? "#c14b3f" : "#8a8078"} />
            <Ledger label="Detective interest" value={`${Math.round(detectiveHeat)}%`} sub={detectiveHeat > 60 ? "Ruiz is close" : "off her radar"} accent={detectiveHeat > 60 ? "#c14b3f" : "#8a8078"} />
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
            ...
            <button onClick={openMeeting} disabled={meetingCooldown > 0} style={{ flex: 1, minWidth: 100, padding: "10px 0", background: meetingCooldown > 0 ? "transparent" : "rgba(212,175,55,0.15)", border: `1px solid ${meetingCooldown > 0 ? "#3a2c20" : "#d4af37"}`, color: meetingCooldown > 0 ? "#5c5142" : "#d4af37", borderRadius: 4, fontFamily: "'Cinzel', serif", fontSize: 11, cursor: meetingCooldown > 0 ? "not-allowed" : "pointer" }}>{meetingCooldown > 0 ? `MEETING IN ${meetingCooldown * 2}s` : "SECRET MEETING ●"}</button>
          </div>
        </div>
      )}

      <div style={{ padding: "0 18px" }} onTouchStart={onContentTouchStart} onTouchEnd={onContentTouchEnd}>
        {swipeHint && (
          <div style={{ textAlign: "center", fontSize: 10.5, color: "#5c5142", fontStyle: "italic", padding: "4px 0 10px" }}>‹ swipe between tabs ›</div>
        )}
        {tab === "empire" && (
          <>
        ...
        {tab === "profile" && (
          <>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, padding: 16, background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 10 }}>
              <div style={{ width: 72, height: 72, borderRadius: 10, background: "linear-gradient(160deg, #d4af3733, #150f0a)", border: "2px solid #d4af37", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 38 }}>{avatar}</div>
              <div>
                <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 24, color: "#e8dcc8" }}>{PLAYER_NAME}</div>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: "#d4af37" }}>{rank.title} · LVL {level}</div>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#8a8078" }}>{wealth.label} · {CITIES.find((c) => c.id === currentCity)?.name}</div>
              </div>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
              {AVATARS.map((a) => (
                <button key={a} onClick={() => setAvatar(a)} style={{ width: 40, height: 40, borderRadius: 8, fontSize: 19, background: avatar === a ? "rgba(212,175,55,0.25)" : "#150f0a", border: `1px solid ${avatar === a ? "#d4af37" : "#2a2018"}`, cursor: "pointer" }}>{a}</button>
              ))}
            </div>

            <SectionTitle>Record</SectionTitle>
            <div style={{ background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 4, padding: "4px 12px" }}>
              <Ledger label="Net worth" value={`$${fmt(cash + bankCash + vaultCash + portfolioValue)}`} accent="#7fae6b" />
            ...
            <SectionTitle tag="live · shared">Global Empires Leaderboard</SectionTitle>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {leaderboard.length === 0 && <div style={{ fontSize: 13, color: "#6b6152", fontStyle: "italic" }}>No standings reported yet. Yours posts automatically as you play.</div>}
              {leaderboard.map((p, i) => (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: p.id === PLAYER_ID ? "rgba(212,175,55,0.12)" : "rgba(20,14,10,0.5)", border: `1px solid ${p.id === PLAYER_ID ? "#d4af37" : "#2a2018"}`, borderRadius: 6 }}>
                  <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 16, color: "#6b6152", width: 22 }}>#{i + 1}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13 }}>{p.name}{p.id === PLAYER_ID && " (you)"}</div>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#8a8078" }}>LVL {p.level} · {p.title}</div>
                  </div>
        ...
        {tab === "crime" && (() => {
          ...
          return (
            <>
              ...
              {petty.length > 0 && (
                <div style={{ fontSize: 12, color: "#6b6152", fontStyle: "italic", marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {(showPettyJobs ? CRIMES : relevant).map((c) => {
                  ...
                  return (
                    <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: 12, background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 4, opacity: locked ? 0.45 : tooSmall ? 0.6 : 1 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#8a8078", marginTop: 3 }}>
                          {tierLocked ? `needs ${WEALTH_CLASSES[Math.min(c.tier - 1, WEALTH_CLASSES.length - 1)]?.label || "more"} standing` : levelLocked ? `requires LVL ${c.level}` : `${c.energy} energy · $${fmt(c.minCash)}-${fmt(c.maxCash)} · risk ${Math.round(c.risk * (1 - muscleCut) * 100)}%`}
        ...
        {tab === "bounties" && (
          <>
            ...
            {activeBounty ? (
              <div style={{ padding: 16, background: "rgba(20,14,10,0.6)", border: "1px solid #d4af3766", borderRadius: 10, textAlign: "center" }}>
                ...
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: "#8a8078", marginBottom: 14 }}>${fmt(activeBounty.reward)} · {activeBounty.energy} energy · +{activeBounty.xp} xp</div>
                <button onClick={completeBounty} disabled={energy < activeBounty.energy} style={{ width: "100%", padding: "11px 0", background: energy < activeBounty.energy ? "#241a13" : "#d4af37", color: "#0e0a07", border: "none", borderRadius: 6, fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>TAKE THE BOUNTY</button>
              </div>
            ) : (
              <div style={{ textAlign: "center", padding: 20 }}>
                <div style={{ fontSize: 13, color: "#6b6152", fontStyle: "italic", marginBottom: 12 }}>{bountyCooldown > 0 ? `A new contract surfaces in ${bountyCooldown * 2}s.` : "A contract just came in."}</div>
                <button onClick={rollBounty} disabled={bountyCooldown > 0} style={{ padding: "10px 20px", background: bountyCooldown > 0 ? "transparent" : "rgba(212,175,55,0.15)", border: `1px solid ${bountyCooldown > 0 ? "#3a2c20" : "#d4af37"}`, color: bountyCooldown > 0 ? "#5c5142" : "#d4af37", borderRadius: 6, fontFamily: "'Cinzel', serif", fontSize: 12, cursor: bountyCooldown > 0 ? "not-allowed" : "pointer" }}>CHECK THE BOARD</button>
              </div>
            )}
          </>
        )}
        ...
        {tab === "assets" && ASSET_CATEGORIES.map((cat) => (
          <div key={cat.id}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {cat.items.map((b) => { const n = owned[b.id] || 0; const price = growPrice(b.cost, n, 1.15, 45); return <AssetTile key={b.id} icon={b.icon} name={b.name} sub={`+$${b.income}/tick`} price={price} owned={n} accent={cat.color} disabled={cash < price} locked={level < cat.level} onBuy={() => buyBusiness(b, cat)} />; })}
            </div>
          </div>
        ))}
        ...
        {tab === "collection" && (
          <>
            ...
            <div style={{ fontSize: 13, color: "#6b6152", fontStyle: "italic", marginBottom: 12 }}>These are yours to keep, not businesses — no income, one-time purchase, pure statement of who you've become. Separate from the Luxury Trade Business, which is a real income-generating front.</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {PERSONAL_ITEMS.map((item) => {
                const owned_ = !!personalItems[item.id];
                return <AssetTile key={item.id} icon={item.icon} name={item.name} sub={owned_ ? "in your collection" : `+${item.respect} respect`} price={item.cost} owned={owned_ ? 1 : 0} accent="#e8dcc8" disabled={owned_ || cash < item.cost} onBuy={() => buyPersonalItem(item)} />;
              })}
            </div>
          </>
        )}
        ...
        {tab === "bank" && (
          <>
            ...
            <div style={{ fontSize: 14, color: "#8a8078", lineHeight: 1.6, marginBottom: 14 }}>
            ...
            <div style={{ padding: 14, background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 8, marginBottom: 12 }}>
              <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 28, color: "#d4af37" }}>${fmt(bankCash)}</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#6b6152" }}>growing quietly, tick by tick</div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => bankDeposit(Math.round(cash * 0.5))} style={{ flex: 1, padding: "12px 0", background: "#d4af37", color: "#0e0a07", border: "none", borderRadius: 6, fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>DEPOSIT HALF</button>
        ...
        {tab === "vault" && (
          <>
            ...
            <div style={{ fontSize: 14, color: "#8a8078", lineHeight: 1.6, marginBottom: 14 }}>Cash on hand is what the Feds can seize if you're arrested. Anything in the vault stays yours.</div>
            <StampBar pct={(vaultCash / vaultCap) * 100} color="#d4af37" />
            <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
              <button onClick={() => depositVault(Math.round(cash * 0.5))} style={{ flex: 1, padding: "12px 0", background: "#7fae6b", color: "#0e0a07", border: "none", borderRadius: 6, fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>DEPOSIT HALF</button>
          ...
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {HIDEOUTS.map((h) => {
                ...
                return (
                  <button key={h.id} onClick={() => upgradeHideout(h)} style={{ textAlign: "left", display: "flex", alignItems: "center", gap: 12, padding: 12, background: active ? "rgba(212,175,55,0.12)" : "rgba(20,14,10,0.6)", border: `1px solid ${active ? "#d4af37" : "#2a2018"}`, borderRadius: 6, cursor: "pointer", color: "#e8dcc8" }}>
                    <div style={{ fontSize: 26 }}>{h.icon}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: "'Cinzel', serif", fontSize: 14 }}>{h.name}{active && " — CURRENT"}</div>
                      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#8a8078", marginTop: 2 }}>heat decay +{h.heatDecay}/tick · respect +{h.respectBonus}</div>
                    </div>
        ...
        {tab === "crew" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {CREW_ROLES.map((r) => { const n = crew[r.id] || 0; const cost = Math.round(r.cost * Math.pow(1.3, n)); return <AssetTile key={r.id} icon={r.icon} name={r.name} sub={r.effect} price={cost} owned={n} accent="#c9973f" disabled={cash < cost} onBuy={() => hireCrew(r)} />; })}
            </div>
            <SectionTitle>Bodyguards</SectionTitle>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {BODYGUARDS.map((g) => { const n = guards[g.id] || 0; const cost = growPrice(g.cost, n, 1.35, 20); return <AssetTile key={g.id} icon={g.icon} name={g.name} sub={`-${Math.round(g.arrestCut * 100)}% arrest risk`} price={cost} owned={n} accent="#c14b3f" disabled={cash < cost} onBuy={() => hireGuard(g)} />; })}
            </div>
            <div style={{ marginTop: 14, fontSize: 13, color: "#6b6152", fontStyle: "italic" }}>Crew and guards draw wages every tick but cut your risk, damage, heat, and jail odds.</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {EXECUTIVE_STAFF.map((r) => {
                ...
                return <AssetTile key={r.id} icon={r.icon} name={r.name} sub={locked ? `needs LVL ${r.level}` : r.desc} price={r.cost} owned={owned_ ? 1 : 0} accent="#4fb8c9" disabled={owned_ || cash < r.cost} locked={locked} onBuy={() => hireStaff(r)} />;
              })}
            </div>
            <div style={{ marginTop: 10, fontSize: 12, color: "#6b6152", fontStyle: "italic" }}>Executive staff are one-of-a-kind hires — each does something no amount of crew can replace.</div>
          </>
        )}
        ...
        {tab === "training" && (
          <>
            ...
            <div style={{ fontSize: 13, color: "#6b6152", fontStyle: "italic", marginBottom: 12 }}>Permanent upgrades — unlike crew, these never leave and never draw a wage.</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {SCHOOLS.map((s) => {
                const n = training[s.id] || 0;
                const maxed = n >= s.cap;
                const cost = Math.round(s.cost * Math.pow(1.6, n));
                return (
                  <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: 12, background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 6 }}>
                    <div style={{ fontSize: 24 }}>{s.icon}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13 }}>{s.name} <span style={{ color: "#d4af37" }}>{n}/{s.cap}</span></div>
                      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, color: "#8a8078" }}>{s.desc}</div>
                    </div>
                    <button onClick={() => trainSkill(s)} disabled={maxed || cash < cost} style={{ padding: "8px 12px", background: maxed ? "#1a1410" : cash < cost ? "#241a13" : "#d4af37", color: maxed ? "#5c5142" : cash < cost ? "#6b6152" : "#0e0a07", border: "none", borderRadius: 4, fontWeight: 700, fontSize: 11, cursor: maxed || cash < cost ? "not-allowed" : "pointer" }}>{maxed ? "MAXED" : `$${fmt(cost)}`}</button>
                  </div>
                );
              })}
            </div>
        ...
        {tab === "hacking" && (
          <>
            ...
            <div style={{ fontSize: 13, color: "#6b6152", fontStyle: "italic", marginBottom: 12 }}>A rhythm mini-game, nothing more — no real techniques here, just Cipher's fictional network.</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {HACK_TARGETS.map((t) => (
                <button key={t.id} onClick={() => startHack(t)} disabled={jailTicks > 0} style={{ textAlign: "left", display: "flex", alignItems: "center", gap: 12, padding: 12, background: "rgba(20,14,10,0.6)", border: "1px solid #4fb8c944", borderRadius: 6, cursor: "pointer", color: "#e8dcc8" }}>
                  <div style={{ fontSize: 22 }}>💻</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13 }}>{t.name}</div>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#8a8078" }}>difficulty {t.diff} · {t.reward ? `$${fmt(t.reward)} · ` : ""}+{t.intel} intel{t.heatCut ? ` · -${t.heatCut} heat` : ""}</div>
                  </div>
                </button>
              ))}
            </div>
          ...
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {FACTIONS.map((f) => { const v = rep[f.id] || 0; return (
                <div key={f.id} style={{ padding: 12, background: "rgba(20,14,10,0.6)", border: `1px solid ${f.color}44`, borderRadius: 8 }}>
                  ...
                  <div style={{ fontSize: 12.5, color: "#8a8078", fontStyle: "italic", margin: "6px 0 8px" }}>{f.desc}</div>
        ...
        {tab === "market" && (
          <>
            ...
            {(() => {
              ...
              return (
                <div style={{ background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 8, padding: 12, marginBottom: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                    <span style={{ fontSize: 18 }}>{selected.icon}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13 }}>{selected.name}</div>
                      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: "#6b6152" }}>{selected.sector} · div {(selected.div * 100).toFixed(2)}%/tick</div>
                    </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {shares.map((s) => { const n = portfolio[s.id] || 0; const selected = (chartCompany || shares[0].id) === s.id; return (
                <div key={s.id} onClick={() => setChartCompany(s.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: 10, background: selected ? "rgba(212,175,55,0.1)" : "rgba(20,14,10,0.5)", border: `1px solid ${selected ? "#d4af37" : "#2a2018"}`, borderRadius: 6, cursor: "pointer" }}>
                  <div style={{ fontSize: 18 }}>{s.icon}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: "'Cinzel', serif", fontSize: 12.5 }}>{s.name} {n > 0 && <span style={{ color: "#d4af37" }}>· {n}</span>}</div>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: "#6b6152" }}>{s.sector}</div>
                  </div>
                  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: "#7fae6b" }}>${s.price.toFixed(2)}</div>
                  <button onClick={(e) => { e.stopPropagation(); buyShare(s); }} disabled={cash < s.price} style={{ padding: "6px 8px", background: cash < s.price ? "#241a13" : "#7fae6b", color: "#0e0a07", border: "none", borderRadius: 4, fontWeight: 700, fontSize: 10, cursor: "pointer" }}>BUY</button>
                  <button onClick={(e) => { e.stopPropagation(); sellShare(s); }} disabled={n <= 0} style={{ padding: "6px 8px", background: n <= 0 ? "#241a13" : "#c14b3f", color: n <= 0 ? "#6b6152" : "#e8dcc8", border: "none", borderRadius: 4, fontWeight: 700, fontSize: 10, cursor: "pointer" }}>SELL</button>
                </div>
              ); })}
            </div>
            <div style={{ fontSize: 12, color: "#6b6152", fontStyle: "italic", marginTop: 8 }}>Dividends pay automatically into cash on hand every tick, based on shares held — no need to sell.</div>

            <SectionTitle>Foreign Exchange Desk</SectionTitle>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {fx.map((s) => { const n = fxHoldings[s.id] || 0; return (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: 12, background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 4 }}>
                  ...
                  <button onClick={() => buyFx(s)} disabled={cash < 1000 * s.price} style={{ padding: "6px 9px", background: "#7fa8c9", color: "#0e0a07", border: "none", borderRadius: 4, fontWeight: 700, fontSize: 10, cursor: "pointer" }}>BUY 1K</button>
                  <button onClick={() => sellFx(s)} disabled={n < 1000} style={{ padding: "6px 9px", background: n < 1000 ? "#241a13" : "#c14b3f", color: n < 1000 ? "#6b6152" : "#e8dcc8", border: "none", borderRadius: 4, fontWeight: 700, fontSize: 10, cursor: "pointer" }}>SELL 1K</button>
                </div>
              ); })}
            </div>
          </>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: 320, overflowY: "auto", paddingRight: 4 }}>
              {chatMessages.map((m) => { const c = CONTACTS.find((x) => x.id === m.contactId); return (
                <div key={m.id} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#241a13", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>{c?.icon}</div>
                  <div style={{ background: "rgba(20,14,10,0.7)", border: "1px solid #2a2018", borderRadius: "3px 10px 10px 10px", padding: "8px 12px", maxWidth: "80%" }}>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#6b6152", marginBottom: 2 }}>{c?.name}</div>
                    <div style={{ fontSize: 14, color: "#e8dcc8", lineHeight: 1.4 }}>{m.text}</div>
                  </div>
                </div>
              ); })}
            </div>

            <SectionTitle tag="live · shared with other players">Global Wire</SectionTitle>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 260, overflowY: "auto", paddingRight: 4, marginBottom: 10 }}>
              {globalChat.length === 0 && <div style={{ fontSize: 13, color: "#6b6152", fontStyle: "italic" }}>No chatter yet — be the first voice on the wire.</div>}
              {globalChat.map((m) => (
                <div key={m.id} style={{ fontSize: 13.5, lineHeight: 1.4 }}>
                  <span style={{ color: m.from === PLAYER_NAME ? "#d4af37" : "#7fa8c9", fontFamily: "'JetBrains Mono', monospace", fontSize: 11 }}>{m.from}: </span>
                  <span style={{ color: "#e8dcc8" }}>{m.text}</span>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <input value={chatDraft} onChange={(e) => setChatDraft(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendGlobalChat()} placeholder="Say something on the wire..." style={{ flex: 1, padding: "9px 10px", background: "#140e0a", border: "1px solid #2a2018", borderRadius: 6, color: "#e8dcc8", fontSize: 13 }} />
              <button onClick={sendGlobalChat} style={{ padding: "9px 14px", background: "#d4af37", color: "#0e0a07", border: "none", borderRadius: 6, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>SEND</button>
            </div>
          </>
        )}
        ...
        {tab === "trade" && (
          <>
            <SectionTitle tag="live · shared marketplace">Player Trade Board</SectionTitle>
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              <input value={tradeDraft.item} onChange={(e) => setTradeDraft((d) => ({ ...d, item: e.target.value }))} placeholder="Item / info to sell" style={{ flex: 2, padding: "9px 10px", background: "#140e0a", border: "1px solid #2a2018", borderRadius: 6, color: "#e8dcc8", fontSize: 13 }} />
              <input value={tradeDraft.price} onChange={(e) => setTradeDraft((d) => ({ ...d, price: e.target.value.replace(/\D/g, "") }))} placeholder="Price" style={{ width: 90, padding: "9px 10px", background: "#140e0a", border: "1px solid #2a2018", borderRadius: 6, color: "#e8dcc8", fontSize: 13 }} />
              <button onClick={postTradeOffer} style={{ padding: "9px 14px", background: "#7fae6b", color: "#0e0a07", border: "none", borderRadius: 6, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>LIST</button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {tradeBoard.length === 0 && <div style={{ fontSize: 13, color: "#6b6152", fontStyle: "italic" }}>Board's empty. List something for other players to buy.</div>}
              {[...tradeBoard].reverse().map((o) => (
                <div key={o.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 6 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13 }}>{o.item}</div>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#6b6152" }}>listed by {o.from}</div>
                  </div>
                  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, color: "#d4af37" }}>${fmt(o.price)}</div>
                  {o.from !== PLAYER_NAME && <button onClick={() => buyTradeOffer(o)} disabled={cash < o.price} style={{ padding: "6px 10px", background: cash < o.price ? "#241a13" : "#d4af37", color: "#0e0a07", border: "none", borderRadius: 4, fontWeight: 700, fontSize: 11, cursor: "pointer" }}>BUY</button>}
                </div>
              ))}
            </div>
          </>
          ...
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {CITIES.map((c) => {
                ...
                return (
                  <button key={c.id} onClick={() => travelTo(c)} style={{ textAlign: "left", padding: 12, background: active ? "rgba(200,30,58,0.12)" : "rgba(20,14,10,0.6)", border: `1px solid ${active ? "#c81e3a" : "#2a2018"}`, borderRadius: 6, cursor: "pointer", color: "#e8dcc8" }}>
                    ...
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#6b6152", marginTop: 2 }}>{c.country} · {mode.label} by {c.mode}</div>
                    <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: 13, color: "#8a8078", marginTop: 3 }}>{c.desc}</div>
                  </button>
                );
              })}
            </div>
        ...
        {tab === "settings" && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ padding: 12, background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 6 }}>
                <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13, marginBottom: 4 }}>Petty jobs visibility</div>
                <div style={{ fontSize: 12, color: "#8a8078", marginBottom: 8 }}>Show crimes below your wealth tier in the Crime tab by default.</div>
                <button onClick={() => setShowPettyJobs((v) => !v)} style={{ padding: "8px 14px", background: showPettyJobs ? "#d4af37" : "transparent", color: showPettyJobs ? "#0e0a07" : "#d4af37", border: "1px solid #d4af37", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{showPettyJobs ? "SHOWING ALL JOBS" : "HIDING PETTY JOBS"}</button>
              </div>
              <div style={{ padding: 12, background: "rgba(20,14,10,0.6)", border: "1px solid #2a2018", borderRadius: 6 }}>
                <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13, marginBottom: 4 }}>Player identity</div>
                <div style={{ fontSize: 12, color: "#8a8078", marginBottom: 8 }}>{PLAYER_NAME} · used for the shared wire, trade board, and leaderboard.</div>
              </div>
              <div style={{ padding: 12, background: "rgba(20,14,10,0.6)", border: "1px solid #c14b3f55", borderRadius: 6 }}>
                <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13, marginBottom: 4, color: "#c14b3f" }}>Reset run</div>
                <div style={{ fontSize: 12, color: "#8a8078", marginBottom: 8 }}>Wipes this browser session's progress and starts over in Newark. Shared wire/trade/leaderboard data is not affected.</div>
                {resetConfirm ? (
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => window.location.reload()} style={{ flex: 1, padding: "9px 0", background: "#c14b3f", color: "#e8dcc8", border: "none", borderRadius: 6, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>CONFIRM RESET</button>
                    <button onClick={() => setResetConfirm(false)} style={{ flex: 1, padding: "9px 0", background: "transparent", color: "#8a8078", border: "1px solid #2a2018", borderRadius: 6, fontSize: 12, cursor: "pointer" }}>Cancel</button>
                  </div>
                ) : (
      ...
      <div style={{ display: "flex", alignItems: "center", position: "fixed", bottom: 0, left: 0, right: 0, background: "#140e0a", borderTop: "1px solid #2a2018" }}>
        <button onClick={() => scrollTabs(-1)} style={{ flexShrink: 0, width: 26, alignSelf: "stretch", background: "none", border: "none", color: "#6b6152", cursor: "pointer", fontSize: 13 }}>‹</button>
        <div ref={tabBarRef} style={{ display: "flex", overflowX: "auto", padding: "8px 2px", flex: 1, scrollBehavior: "smooth" }}>
          {TABS.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: "1 0 auto", minWidth: 52, background: "none", border: "none", cursor: "pointer", color: tab === t.id ? "#d4af37" : "#5c5142", padding: "6px 2px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
