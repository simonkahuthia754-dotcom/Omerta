...
function AssetTile({ icon, name, sub, price, owned, accent, onBuy, disabled, locked }) {
  return (
    <div style={{
      ...
      background: `linear-gradient(150deg, ${accent}33 0%, #0a0a0a 55%, #0a0705 100%)`,
    }}>
      ...
      <div style={{ padding: "8px 10px 10px", background: "rgba(0,0,0,0.6)", borderTop: `1px solid ${accent}33` }}>
    ...
    <div style={{ minHeight: "100vh", background: "#050505", backgroundImage: "radial-gradient(ellipse at top, #0a0a0a 0%, #050505 60%)", color: "#f2f0ed", fontFamily: "'Inter', sans-serif", paddingBottom: 84 }}>
      ...
      {ledgerOpen && (
        <div style={{ padding: "0 18px 14px", borderBottom: "1px solid #262626" }}>
          ...
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#666666", margin: "6px 0 10px" }}>{fmt(xp - rank.xp)} / {nextRank ? fmt(nextRank.xp - rank.xp) : "MAX"} XP to {nextRank?.title || "the top"} · Playing as {PLAYER_NAME}</div>
        ...
        {tab === "profile" && (
          <>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, padding: 16, background: "rgba(15,15,15,0.7)", border: "1px solid #262626", borderRadius: 10 }}>
              ...
              <div>
                <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 24, color: "#f2f0ed" }}>{PLAYER_NAME}</div>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {PERSONAL_ITEMS.map((item) => {
                ...
                return <AssetTile key={item.id} icon={item.icon} name={item.name} sub={owned_ ? "in your collection" : `+${item.respect} respect`} price={item.cost} owned={owned_ ? 1 : 0} accent="#f2f0ed" disabled={owned_ || cash < item.cost} onBuy={() => buyPersonalItem(item)} />;
              })}
            </div>
          </>
        )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {[...tradeBoard].reverse().map((o) => (
                <div key={o.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "rgba(15,15,15,0.7)", border: "1px solid #262626", borderRadius: 6 }}>
                  ...
                  {o.from !== PLAYER_NAME && <button onClick={() => buyTradeOffer(o)} disabled={cash < o.price} style={{ padding: "6px 10px", background: cash < o.price ? "#1a1a1a" : "#d4af37", color: "#050505", border: "none", borderRadius: 4, fontWeight: 700, fontSize: 11, cursor: "pointer" }}>BUY</button>}
                </div>
              ))}
            </div>
          </>
          ...
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ padding: 12, background: "rgba(15,15,15,0.7)", border: "1px solid #262626", borderRadius: 6 }}>
                ...
                <div style={{ fontSize: 12, color: "#8f8f8f", marginBottom: 8 }}>{PLAYER_NAME} · used for the shared wire, trade board, and leaderboard.</div>
              </div>
              <div style={{ padding: 12, background: "rgba(15,15,15,0.7)", border: "1px solid #c14b3f55", borderRadius: 6 }}>
                ...
                <div style={{ fontSize: 12, color: "#8f8f8f", marginBottom: 8 }}>Wipes this browser session's progress and starts over in Newark. Shared wire/trade/leaderboard data is not affected.</div>
                {resetConfirm ? (
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => window.location.reload()} style={{ flex: 1, padding: "9px 0", background: "#c14b3f", color: "#f2f0ed", border: "none", borderRadius: 6, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>CONFIRM RESET</button>
