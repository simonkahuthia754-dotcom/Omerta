...
function OmertaGame() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      guards, portfolio, fxHoldings, unlockedCities, currentCity, rep, jailTicks, priorArrests, seenLevels]);
  ...
  useEffect(() => {
    const t = setInterval(async () => {
      try {
        ...
        filtered.push({ id: PLAYER_ID, name: playerName, cash: c, level: lv, title: rk.title });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050505", backgroundImage: "radial-gradient(ellipse at top, #0a0a0a 0%, #050505 60%)", color: "#f2f0ed", fontFamily: "'Inter', sans-serif", paddingBottom: 84 }}>
      ...
      <div style={{ padding: "0 18px", touchAction: "pan-y" }} onTouchStart={onContentTouchStart} onTouchEnd={onContentTouchEnd}>
        ...
        {tab === "settings" && (() => {
          const wealthTierNow = wealthTierIndex(cash);
          const adBonus = Math.round((200 + wealthTierNow * 800) * (0.9 + Math.random() * 0.2));
          return (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ padding: 12, background: "rgba(15,15,15,0.7)", border: "1px solid #d4af3755", borderRadius: 6 }}>
                <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13, marginBottom: 4, color: "#d4af37" }}>📺 Watch an ad for a cash bonus</div>
                <div style={{ fontSize: 12, color: "#8f8f8f", marginBottom: 8 }}>Reward scales with your current standing — right now that's about ${fmt(adBonus)}. No cooldown once the ad plugin is active.</div>
                <button onClick={async () => {
                  const rewarded = await adManager.showRewardedAd();
                  if (rewarded) { setCash((c) => c + adBonus); pushLog(`Watched an ad — banked $${fmt(adBonus)}.`, "good"); }
                ...
                }} style={{ width: "100%", padding: "11px 0", background: "#d4af37", color: "#050505", border: "none", borderRadius: 6, fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>WATCH AD — +${fmt(adBonus)}</button>
              </div>
